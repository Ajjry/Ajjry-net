...
def train(config):
    ...
    La_net = Enhance.enhance_net(config.chanle, config.kernel_size, device).cuda()
    ...
    L_TV = utils.L_TV()
    L_TV_low=utils.L_TV_low()
    l2_loss = torch.nn.MSELoss()
    l1_loss = torch.nn.L1Loss()
    if config.resume:
        checkpoint_decom = torch.load(config.resume_checkpoint+"/checkpoint_decom/Epoch"+str(config.resume_epoch)+".pth")
        start_epoch = checkpoint_decom['epoch'] + 1
        decom_net.load_state_dict(checkpoint_decom['model'])
        optimizer_decom.load_state_dict(checkpoint_decom['optimizer'])
        StepLR_decom.load_state_dict(checkpoint_decom['lr_schedule'])  # 加载lr_scheduler

        checkpoint_La= torch.load(config.resume_checkpoint+"/checkpoint_La/Epoch"+str(config.resume_epoch)+".pth")
        La_net.load_state_dict(checkpoint_La['model'])
        optimizer_La.load_state_dict(checkpoint_La['optimizer'])

        checkpoint_DES = torch.load(config.resume_checkpoint+"/checkpoint_DES/Epoch"+str(config.resume_epoch)+".pth")
        DES_net.load_state_dict(checkpoint_DES['model'])
        optimizer_DES.load_state_dict(checkpoint_DES['optimizer'])

        print("=> loaded checkpoint (epoch {})".format(checkpoint_DES['epoch']))
    for epoch in range(config.num_epochs-start_epoch):
        for img_lowlight, ground_truth in tqdm(train_loader):
            ...
            enhance_img_lowlight_low_frequency,_,_,_,kernel,_,_= La_net(img_lowlight_low_frequency)
            ...
            contrast_loss = utils.contrast_loss(enhance_img_lowlight_low_frequency, img_highlight_low_frequency_detach)
            ...
            psnr_train = utils.batch_PSNR(result, ground_truth, 1.)
            ssim_train = utils.SSIM(result, ground_truth)
        ...
        if ((epoch+start_epoch) % config.snapshot_iter) == 0:
            checkpoint_decom = {
                ...
                'model': decom_net.state_dict(),
                'optimizer': optimizer_decom.state_dict(),
                'lr_schedule': StepLR_decom.state_dict(),

            }
            checkpoint_La = {
                'epoch': epoch + start_epoch,
                'model': La_net.state_dict(),
                'optimizer': optimizer_La.state_dict(),
                'lr_schedule': StepLR_La.state_dict(),

            }
            checkpoint_DES = {
                'epoch': epoch + start_epoch,
                'model': DES_net.state_dict(),
                'optimizer': optimizer_DES.state_dict(),
                'lr_schedule': StepLR_DES.state_dict(),
            }
            snapshots_folder = config.snapshots_folder.replace("snapshots3","snapshots3_"+config.train_model)
            if not os.path.isdir(snapshots_folder):
                os.mkdir(snapshots_folder)
            if not os.path.isdir(os.path.join(snapshots_folder,'checkpoint_decom')):
                os.mkdir(os.path.join(snapshots_folder,'checkpoint_decom'))
            if not os.path.isdir(os.path.join(snapshots_folder,'checkpoint_La')):
                os.mkdir(os.path.join(snapshots_folder,'checkpoint_La'))
            if not os.path.isdir(os.path.join(snapshots_folder,'checkpoint_DES')):
                os.mkdir(os.path.join(snapshots_folder,'checkpoint_DES'))
            torch.save(checkpoint_decom, os.path.join(snapshots_folder,'checkpoint_decom/') + "Epoch" + str(epoch + start_epoch) + '.pth')
            torch.save(checkpoint_La, os.path.join(snapshots_folder,'checkpoint_La/') + "Epoch" + str(epoch + start_epoch) + '.pth')
            torch.save(checkpoint_DES, os.path.join(snapshots_folder, 'checkpoint_DES/') + "Epoch" + str(epoch + start_epoch) + '.pth')
        StepLR_decom.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lowlight_images_path', type=str,
                        default="/home/cc/CODE/Dehazing_net_code/data/train/OTS/haze")
    parser.add_argument('--ground_truth_path', type=str, default="/home/cc/CODE/Dehazing_net_code/data/train/OTS/gt")
    ...
    parser.add_argument('--Step_size', type=float, default=10)
    parser.add_argument('--weight_decay', type=float, default=1e-4)
    parser.add_argument('--Step_gama', type=float, default=0.5)
    parser.add_argument('--chanle', type=int, default=3)
    parser.add_argument('--kernel_size', type=int, default=11)

    parser.add_argument('--resume', type=bool, default=False)
    parser.add_argument('--resume_checkpoint', type=str,
                        default='/home/cc/CODE/Contrast_net_code/snapshots')
    ...
    parser.add_argument('--snapshots_folder', type=str, default="/home/cc/CODE/Contrast_net_code/snapshots3")
