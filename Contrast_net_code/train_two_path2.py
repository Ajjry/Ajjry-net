...
def train(config):
    ...
    for epoch in range(config.num_epochs-start_epoch):
        for img_lowlight, ground_truth in tqdm(train_loader):
            ...
            loss_enh=L2_loss_enh + 2 * contrast_loss + kernel_loss
        ...
        if ((epoch+start_epoch) % config.snapshot_iter) == 0:
            ...
            snapshots_folder = config.snapshots_folder.replace("snapshots","snapshots_"+config.train_model)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--snapshots_folder', type=str, default="/home/cc/CODE/Contrast_net_code/snapshots")
