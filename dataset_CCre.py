...
class ColorCheckerRE(data.Dataset):
    def __init__(self,train=True,folds_num=1):
        # self.root = 'E:/ETHZ/ColorConstancy/Datasets/ColorCheckerRECommended/colorchecker/' # ./data/ColorCheckerRe/
        # self.root = './data/ColorCheckerRe/'
        self.root ='/home/yangkf/GPnet/data/ColorCheckerRe/'
        ...
        self.illums = scipy.io.loadmat( self.root  +'./groundtruthcoordinates/ColorCheckerData.mat')  
    ...
    def __getitem__(self,index):
        ...
        img = cv2.imread(self.root  +'./images/'+fn,-1)

        num = fn.index('_')
        illums = self.illums['REC_groundtruth'][int(fn[0:num])-1]
        ...
        mask1 = cv2.imread(self.root  +'./masks/'+'mask1'+ fn[num:],0)
        ret,mask1 = cv2.threshold(mask1,100,255,cv2.THRESH_BINARY)
        mask2 = cv2.imread(self.root  +'./masks/'+'mask2'+ fn[num:],0)
        ret,mask2 = cv2.threshold(mask2,100,255,cv2.THRESH_BINARY)
        mask3 = cv2.imread(self.root  +'./masks/'+'mask3'+ fn[num:],0)
        ret,mask3 = cv2.threshold(mask3,100,255,cv2.THRESH_BINARY)
...
if __name__=='__main__':
    ...
    dataset = ColorCheckerRE(train=True,folds_num=0)
    dataload = torch.utils.data.DataLoader(dataset, batch_size=1,shuffle=False)    
    for ep in range(1):
        # time1 = time.time()
