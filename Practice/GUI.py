...
"background-color: rgb(97, 221, 255);")
    ...
    def retranslateUi(self, BloodVessel):
        ...
        BloodVessel.setWindowTitle(_translate("BloodVessel", "Retinal fundus image enhancement"))
        self.label.setText(_translate("BloodVessel", "<html><head/><body><p><span style=\" font-size:18pt; font-weight:600; font-style:italic; color:#75507b;\">Retinal fundus image enhancement and    Blood Vessel Segmentation</span></p></body></html>"))
        self.InputConsole.setTitle(_translate("BloodVessel", "Console"))
        self.LoadImageButton.setText(_translate("BloodVessel", "Load Image"))
        self.SegmentButton.setText(_translate("BloodVessel", "Enhance and Segment"))
        self.ResetButton.setText(_translate("BloodVessel", "Reset"))
        self.label_2.setText(_translate("BloodVessel", "Input Image"))
        self.label_3.setText(_translate("BloodVessel", "Input Image matchFilter"))
        self.label_4.setText(_translate("BloodVessel", "Enhanced Image matchFilter"))
        self.label_6.setText(_translate("BloodVessel", "Enhanced Image"))
        self.label_7.setText(_translate("BloodVessel", "Enhanced Image U-net"))
        self.label_8.setText(_translate("BloodVessel", "Input Image U-net"))
        self.label_9.setText(_translate("BloodVessel", "Enhanced Image threshold"))
        self.label_10.setText(_translate("BloodVessel", "Input Image threshold"))
# End of GUI Initialization
    ...
    def on_SegmentButton_clicked(self):
        ...
        print(fileName.replace('input.png','0.png'))
        result=cv.imread(fileName.replace('input.png','0.png'))
        result=cv.cvtColor(result,cv.COLOR_BGR2RGB)
        g = cv.resize(result, (211, 211))
        h, w,c  = g.shape
        bytesPerLine = c * w
        convertToQtFormat = QtGui.QImage(g, w, h, bytesPerLine, QtGui.QImage.Format_RGB888)
        pixmap = QPixmap(convertToQtFormat)
        self.GreenLabel.setPixmap(pixmap)

        matchFilterin=cv.imread(fileName.replace('input.png','input_matchFilter.png'))
        matchFilterin=255-matchFilterin
        th = cv.resize(matchFilterin, (211, 211))
        h, w ,c= th.shape
        bytesPerLine =c* w
        convertToQtFormat = QtGui.QImage(th, w, h, bytesPerLine, QtGui.QImage.Format_RGB888)
        pixmap = QPixmap(convertToQtFormat)
        self.AdapThreshLabel.setPixmap(pixmap)

        matchFilter0=cv.imread(fileName.replace('input.png','0_matchFilter.png'))
        matchFilter0=255-matchFilter0
        not_th = cv.resize(matchFilter0, (211, 211))
        h, w ,c= not_th.shape
        bytesPerLine = w*c
        convertToQtFormat = QtGui.QImage(not_th, w, h, bytesPerLine, QtGui.QImage.Format_RGB888)
        pixmap = QPixmap(convertToQtFormat)
        self.InvertLabel.setPixmap(pixmap)

        thresholdin=cv.imread(fileName.replace('input.png','input-1.png'))

        sharpened = cv.resize(thresholdin, (211, 211))
        h, w,c = sharpened.shape
        bytesPerLine = w*c
        convertToQtFormat = QtGui.QImage(sharpened, w, h, bytesPerLine, QtGui.QImage.Format_RGB888)
        pixmap = QPixmap(convertToQtFormat)
        self.SharpLabel.setPixmap(pixmap)

        threshold0 = cv.imread(fileName.replace('input.png', '0-1.png'))

        b = cv.resize(threshold0, (211, 211))
        h, w ,c= b.shape
        bytesPerLine = w*c
        convertToQtFormat = QtGui.QImage(b, w, h, bytesPerLine, QtGui.QImage.Format_RGB888)
        pixmap = QPixmap(convertToQtFormat)
        self.WeinerLabel.setPixmap(pixmap)

        unetin= cv.imread(fileName.replace('input.png', '0input_unet.png'))
        th3 = cv.resize(unetin, (211, 211))
        h, w ,c= th3.shape
        bytesPerLine = w*c
        convertToQtFormat = QtGui.QImage(th3, w, h, bytesPerLine, QtGui.QImage.Format_RGB888)
        pixmap = QPixmap(convertToQtFormat)
        self.OtsuLabel.setPixmap(pixmap)

        unet0= cv.imread(fileName.replace('input.png', '0_unet.png'))
        opening = cv.resize(unet0, (211, 211))
        h, w,c = opening.shape
        bytesPerLine = w*c
        convertToQtFormat = QtGui.QImage(opening, w, h, bytesPerLine, QtGui.QImage.Format_RGB888)
        pixmap = QPixmap(convertToQtFormat)
