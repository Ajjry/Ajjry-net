...
Step1_Search_Window = 39                # 在以NS*NS为中心的限制大小的当前块邻域中搜索候选匹配块
...
def Step1_fast_match(_noisyImg, _BlockPoint):
    ...
    *_BlockPoint:当前block的坐标及大小
    ...
    Search_Step = Step1_Search_Step#块的搜索步长：3
    ...
    similar_blocks = numpy.zeros((blk_num**2, Blk_Size, Blk_Size), dtype=float)#相似块
    m_Blkpositions = numpy.zeros((blk_num**2, 2), dtype=int)#块的坐标
...
def Step2_fast_match(_Basic_img, _noisyImg, _BlockPoint):
    ...
    *_BlockPoint:当前block的坐标及大小
    ...
    Window_location = Define_SearchWindow(_noisyImg, _BlockPoint, Window_size, Blk_Size)#获取窗口顶点
    blk_num = (Window_size-Blk_Size)/Search_Step  # 确定最多可以找到多少相似blk
    blk_num = int(blk_num)
    (present_x, present_y) = Window_location
...
def BM3D(img):
    ...
    if min(img_shape)>4:
       ...
       for i in range(2):
           imgB = img
           imgF = img
           Basic_img = BM3D_1st_step(img[:,:,i])
           imgB[:,:,i]=Basic_img
           Final_img = BM3D_2nd_step(Basic_img, img[:,:,i])
           imgF[:,:,i]=Final_img
       return imgF
    e2 = cv2.getTickCount()
